    ...
    <div className="space-y-4">
      {deptOrderDirty && (
        ...
        >
          <span className="text-sm" style={{ color: "var(--th-text-primary)" }}>
            {tr("순번이 변경되었습니다.", "Order has been changed.", "順番が変更されました。", "顺序已更改。")}
          </span>
          <button
            onClick={onSaveOrder}
          ...
          >
            {reorderSaving ? tr("저장 중...", "Saving...", "保存中...", "保存中...") : tr("순번 저장", "Save Order", "順番を保存", "保存顺序")}
          </button>
          <button
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    ...
                    style={{ background: `${dept.color}22`, color: dept.color }}
                  >
                    {agentCountForDept} {tr("명", "agents", "名", "人")}
                  </span>
                </div>
                {dept.description && (
      ...
      {deptOrder.length === 0 && (
        <div className="text-center py-16" style={{ color: "var(--th-text-muted)" }}>
          ...
          {tr("등록된 부서가 없습니다.", "No departments found.", "登録された部署がありません。", "暂无部门。")}
        </div>
      )}
    </div>
  );
