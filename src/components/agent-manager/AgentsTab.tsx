  ...
  return (
    <>
      <div className="grid grid-cols-3 gap-3">
        {[
          {
            label: tr("전체 인원", "Total", "全員", "总人数"),
            value: agents.length,
            icon: <StackedSpriteIcon sprites={randomIconSprites.total} />,
          },
          { label: tr("근무 중", "Working", "稼働中", "工作中"), value: workingCount, icon: "💼" },
          { label: tr("부서", "Departments", "部署", "部门"), value: departments.length, icon: "🏢" },
        ].map((summary) => (
          <div
            key={summary.label}
      ...
      <div className="flex items-center gap-2 flex-wrap" style={{ borderBottom: "1px solid var(--th-card-border)" }}>
        ...
        >
          {tr("전체", "All", "全体", "全部")} <span className="opacity-60">{agents.length}</span>
        </button>
        {departments.map((department) => {
          ...
          return (
            <button
              ...
              title={tr("더블클릭: 부서 편집", "Double-click: edit dept", "ダブルクリック: 部署編集", "双击编辑部门")}
              className={`flex items-center gap-1 px-3 py-2 text-xs font-medium transition-colors ${
      ...
      {sortedAgents.length === 0 ? (
        <div className="text-center py-16" style={{ color: "var(--th-text-muted)" }}>
          ...
          {tr("검색 결과 없음", "No agents found", "検索結果なし", "未找到员工")}
        </div>
      ) : (
