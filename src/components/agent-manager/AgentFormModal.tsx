      ...
      >
        ...
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-base font-bold" style={{ color: "var(--th-text-heading)" }}>
            {isEdit ? tr("직원 정보 수정", "Edit Agent", "エージェント情報編集") : tr("신규 직원 채용", "Hire New Agent", "新規エージェント採用")}
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="grid grid-cols-2 gap-5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-2">
                  <label className="block text-xs mb-1.5 font-medium" style={{ color: "var(--th-text-secondary)" }}>
                    {tr("영문 이름", "Name", "英語名")} <span className="text-red-400">*</span>
                  </label>
                  <input
                    type="text"
                    value={form.name}
            ...
            <div className="grid grid-cols-[72px_1fr] gap-2">
              ...
              <div>
                ...
                <select
                  ...
                  style={inputStyle}
                >
                  <option value="">{tr("— 미배정 —", "— Unassigned —", "— 未配属 —")}</option>
                  {departments.map((d) => (
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-xs mb-1.5 font-medium" style={{ color: "var(--th-text-secondary)" }}>
                {tr("성격 / 역할 프롬프트", "Personality / Prompt", "性格 / 役割プロンプト")}
              </label>
              <textarea
                value={form.personality}
                onChange={(e) => setForm({ ...form, personality: e.target.value })}
                rows={3}
                placeholder={tr("전문 분야나 성격 설명...", "Expertise or personality...", "専門分野や性格の説明...")}
        ...
        <div className="mt-5 pt-4" style={{ borderTop: "1px solid var(--th-card-border)" }}>
          <div
            ...
            style={{ color: "var(--th-text-muted)" }}
          >
            {tr("캐릭터 스프라이트", "Character Sprite", "キャラクタースプライト")}
          </div>
          ...
          {!previews && !processing && (
            ...
            >
              ...
              <span className="text-xs">
                {tr("4방향 스프라이트 시트 업로드 (2x2 그리드)", "Upload 4-direction sprite sheet (2x2 grid)", "4方向スプライトシートアップロード (2x2グリッド)")}
              </span>
              <span className="text-xs">{tr("앞 / 왼 / 뒤 / 오른 순서", "Front / Left / Back / Right order", "前 / 左 / 後 / 右 の順")}</span>
              <span className="text-xs">
                {t({
                  ko: "(흰색배경)",
                  en: "(White background)",
          ...
          {processing && (
            <div className="flex items-center justify-center gap-2 py-8" style={{ color: "var(--th-text-muted)" }}>
              ...
              <span className="text-sm">
                {tr("배경 제거 및 분할 처리 중...", "Removing background & splitting...", "背景除去・分割処理中...")}
              </span>
            </div>
          )}
          ...
          {previews && !processing && (
            <div className="space-y-3">
                ...
                {(["D", "L", "R"] as const).map((dir) => (
                  <div key={dir} className="text-center">
                    <div className="text-[10px] font-medium mb-1" style={{ color: "var(--th-text-muted)" }}>
                      {dir === "D" ? tr("정면", "Front", "正面") : dir === "L" ? tr("좌측", "Left", "左側") : tr("우측", "Right", "右側")}
                    </div>
                    <div
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  {registering
                    ? tr("등록 중...", "Registering...", "登録中...")
                    : registered
                      ? tr("등록 완료!", "Registered!", "登録完了!")
                      : tr("스프라이트 등록", "Register Sprite", "スプライト登録")}
                </button>
                {previews && (
                  <button
                    onClick={() => {
        ...
        <div className="flex gap-2 mt-5 pt-4" style={{ borderTop: "1px solid var(--th-card-border)" }}>
          ...
          >
            {saving
              ? tr("처리 중...", "Saving...", "処理中...")
              : isEdit
                ? tr("변경사항 저장", "Save Changes", "変更を保存")
                : tr("채용 확정", "Confirm Hire", "採用確定")}
          </button>
          <button
            onClick={onClose}
