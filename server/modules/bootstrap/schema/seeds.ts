...
export function applyDefaultSeeds(db: DbLike): void {
  ...
  if (agentCount === 0) {
    ...
    // Development (3)
    insertAgent.run(randomUUID(), "Aria", "아리아", "dev", "team_leader", "claude", "👩‍💻", "丁寧なシニア開発者");
    insertAgent.run(randomUUID(), "Bolt", "볼트", "dev", "senior", "codex", "⚡", "高速コーディングの専門家");
    insertAgent.run(randomUUID(), "Nova", "노바", "dev", "junior", "copilot", "🌟", "クリエイティブなジュニア");
    // Design (2)
    ...
    insertAgent.run(randomUUID(), "Clio", "클리오", "planning", "senior", "claude", "📝", "データ駆動型プランナー");
    // Operations (2)
    insertAgent.run(randomUUID(), "Atlas", "아틀라스", "operations", "team_leader", "claude", "🗺️", "運営のプロ");
    ...
    insertAgent.run(randomUUID(), "Lint", "린트", "qa", "senior", "codex", "🔬", "丁寧なテストの専門家");
    // DevSecOps (2)
    ...
    // QA Junior (1)
    insertAgent.run(randomUUID(), "DORO", "도로롱", "qa", "junior", "gemini", "🩷", "丁寧な品質管理ジュニア");
  ...
  // Migrate: add sort_order column & set correct ordering for existing DBs
  {
    ...
    const newAgents: [string, string, string, string, string, string, string][] = [
      ...
      ["Clio", "클리오", "planning", "senior", "claude", "📝", "データ駆動型プランナー"],
      ["Turbo", "터보", "operations", "senior", "codex", "🚀", "自動化の専門家"],
      ["Hawk", "호크", "qa", "team_leader", "claude", "🦅", "鋭い品質監視者"],
      ["Lint", "린트", "qa", "senior", "opencode", "🔬", "丁寧なテストの専門家"],
